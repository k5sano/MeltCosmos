...
/// Dattorro reverb tank (loop section only).
/// Extracted from DattorroReverb: the two cross-coupled
/// allpass+delay loops (side A and side B).
/// Returns stereo wet outputs + tail signal for cross-feed.
struct TankOutput {
    float wetL = 0.0f;
    float wetR = 0.0f;
    float tail = 0.0f;  // cross-feed source
};
