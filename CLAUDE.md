```markdown
Copy# EMSpace — Claude Code プロジェクト設定

## プロジェクト概要

EarthQuaker Devices Avalanche Run にインスパイアされた空間系 VST3 プラグイン。
EMverb の Dattorro plate reverb を内部分解し、Mod Delay Line を
Input Diffusion と Reverb Tank の間に挿入。Reverb tail → Delay feedback の
cross-feed により唯一無二の空気感を生む。
...
- プラグインターゲットには `juce_add_plugin()` を使い、`FORMATS` に必要なフォーマットをリストすること。
